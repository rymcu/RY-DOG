...
void power_amplifier_on(void)
{
    ...
    {
        gpio_set_output_high_level(PC,pin_4);
    }
    else
    {
...
void pad_config_for_power_amplifier(void)
{
    ...
    dpmu_set_io_direction(PC4,DPMU_IO_DIRECTION_INPUT);
    dpmu_set_io_pull(PC4,DPMU_IO_PULL_DISABLE);
	dpmu_set_io_reuse(PC4,SECOND_FUNCTION);
    gpio_set_input_mode(PC,pin_4);
    uint8_t pa_default_level = gpio_get_input_level(PC,pin_4);
