...
void pad_config_for_uart(UART_TypeDef *UARTx)
{
    ...
    ///tag-user-defined-uart-init-by-uart-num-start
    //tag_user_defined_uart_init_by_uart_num
    
    ///tag-user-defined-uart-init-by-uart-num-end
}
...
///tag-user-defined-pa-init-by-pin-num-start
//tag_user_defined_pa_init_by_pin_num

///tag-user-defined-pa-init-by-pin-num-end
...
const cm_codec_hw_info_t host_mic_hw_info = 
{
   ...
   .codec_gain.codec_adc_input_mode_l = INNER_CODEC_INPUT_MODE_DIFF,//差分
#else
   .codec_gain.codec_adc_input_mode_l = INNER_CODEC_INPUT_MODE_SINGGLE_ENDED,//单端
#endif 
    #if USE_AEC_MODULE
...
#if (MIC_DIFF_SINGLE == 0)
    .codec_gain.dig_gain_l = 1.0f,//差分
#else
    .codec_gain.dig_gain_l = 6.0f,//单端//数字增益配置，该值加/减 1 ,数字增益加/减 1dB
#endif 
