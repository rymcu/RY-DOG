...
void pad_config_for_uart(UART_TypeDef *UARTx)
{
    ...
    else if (UARTx == UART1)
    {
        ...
        dpmu_set_io_pull(PA2,DPMU_IO_PULL_DISABLE);  //RX关闭上拉，使用外部上拉
        dpmu_set_io_pull(PA3,DPMU_IO_PULL_DISABLE);  //RX关闭上拉，使用外部上拉
    ...
    else if (UARTx == UART2)
    {
        ...
        dpmu_set_io_pull(PA5,DPMU_IO_PULL_DISABLE);  //RX关闭上拉，使用外部上拉
        dpmu_set_io_pull(PA6,DPMU_IO_PULL_DISABLE);  //RX关闭上拉，使用外部上拉
...
void pad_config_for_iis(void)
{
    ...
    //dpmu_set_io_reuse(PA5,IISx_IO_reuse);  因为该板子功放控制脚为PA5，所以屏蔽该脚的IIS功能
...
void power_amplifier_on(void)
{
    ...
    {
        gpio_set_output_high_level(PA,pin_5);
    }
    else
    {
        gpio_set_output_low_level(PA,pin_5);
    }
}
...
void pad_config_for_power_amplifier(void)
{
    ...
    dpmu_set_io_direction(PA5,DPMU_IO_DIRECTION_INPUT);
    dpmu_set_io_pull(PA5,DPMU_IO_PULL_DISABLE);
	dpmu_set_io_reuse(PA5,FIRST_FUNCTION);
    gpio_set_input_mode(PA,pin_5);
    uint8_t pa_default_level = gpio_get_input_level(PA,pin_5);
