...
void pad_config_for_iis(void)
{
    ...
    //dpmu_set_io_reuse(PA4,IISx_IO_reuse);    因为该板子功放控制脚为PA4，所以屏蔽该脚的IIS功能
...
void power_amplifier_on(void)
{
    ...
    {
        gpio_set_output_high_level(PA,pin_4);
    }
    else
    {
        gpio_set_output_low_level(PA,pin_4);
    }
}
...
void pad_config_for_power_amplifier(void)
{
    ...
    dpmu_set_io_pull(PA4,DPMU_IO_PULL_DISABLE);
	dpmu_set_io_reuse(PA4,FIRST_FUNCTION);
    gpio_set_input_mode(PA,pin_4);
    uint8_t pa_default_level = gpio_get_input_level(PA,pin_4);
    dpmu_set_io_direction(PA4,DPMU_IO_DIRECTION_OUTPUT);
